...
<template>
  <section class="page">
        <div class="app">
            ...
            <div class="filter">
              <span>Filter :</span>
              <button class="ed" :class="{ active: filter == 'all' }" @click="filter = 'all'">All</button>
              <button class="ed" :class="{ active: filter == 'active' }" @click="filter = 'active'">Active</button>
              <button class="ed" :class="{ active: filter == 'completed' }" @click="filter = 'completed'">Completed</button>
            </div>
            <p v-if="tasks.length === 0">Here is Your Task</p>
            <ul>
                <li class="task-item" v-for="task in tasksFiltered" :key="task.id">
                    <input type="checkbox" @click="toggleDone(task)" :checked="task.isCompleted">
                    <span :class="{ done: task.isCompleted }">{{ task.name }}</span>
                    <i class="fa-solid fa-trash-can" title="Delete Task" @click="delTask(task)"></i>
                </li>
            </ul>
        </div>
    </section>
